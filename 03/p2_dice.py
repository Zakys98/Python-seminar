...
def zip_with(func, list_1, list_2) -> list:
...
def pair_with(func, items):
    output = []
    i = 0
    while len(items) > i + 1:
        output.append(func(items[i], items[i+1]))
        i += 2
    return output
...
def zip_n_with(func, *args):
    output = []
    for list in zip(*args):
        for x in zip(*[iter(list)]*len(list)):
            output.append(func(*x))
    return output

def chunk_with(func, chunk_size, items):
    output = []
    i = 0
    while len(items) > i + chunk_size - 1:
        output.append(func(*items[i:i + chunk_size]))
        i += chunk_size
    return output
