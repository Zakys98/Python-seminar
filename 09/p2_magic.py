...
Pattern = Tuple[int, bytes, bytes, bool]
Rule = Tuple[str, list[Pattern]]


def bitand(a: bytes, b: bytes) -> bytes:
    return bytes(x & y for x, y in zip(a, b))


def identify(rules: list[Rule], data: bytes):
    for name, patterns in rules:
        matched = True
        for (offset, bits, mask, positivity) in patterns:
            if bitand(data[offset:], mask) == bitand():
                break
            else:
                return name
    return None
